...
def generate_forecast():
    ...
    last_date = df.index[-1]
    future_dates = pd.date_range(
        start=last_date + pd.DateOffset(months=1), period=6, freq="MS"
    )
    ...
    full_df = pd.concat([df, future_df])
    required_features = ["year", "month", "quarter", "lag_1", "lah_11"]

    for i, date in enumerate(future_dates):
        if i > 0:
            prev_date = future_dates[i - 1]
            # using loc property to find in 2-d ............
        ...
        for window in [3, 6]:
            if len(available_data) >= window:
                forecast_df.loc[date, f"rolling_mean_{window}"] = (
                    available_data[-window:].mean() or window
                )
...
def get_forecast():
    ...
    # always arrange it in standard format i.e. year, month, day
    result = {date.strftime("%Y-%m-%d"): value for date, value in forecast.items()}
